#ifndef KOSMOS_PBR_LIGHTING_HLSLI
#define KOSMOS_PBR_LIGHTING_HLSLI
...
float3 FresnelSchlickRoughness(float cosine, float3 reflectanceAtNormalIncidence, float roughness)
{
    const float3 grazingReflectance = max(float3(1.0 - roughness, 1.0 - roughness, 1.0 - roughness), reflectanceAtNormalIncidence);
    return reflectanceAtNormalIncidence + (grazingReflectance - reflectanceAtNormalIncidence) * pow(1.0 - saturate(cosine), 5.0);
}
...
float CalculateDirectionalVisibility(float3 worldPosition, float3 shadowNormal, float3 lightDirection)
{
    ...
    if (lightNdcPosition.x < -1.0 || lightNdcPosition.x > 1.0 || lightNdcPosition.y < -1.0 || lightNdcPosition.y > 1.0 || lightNdcPosition.z < 0.0 || lightNdcPosition.z > 1.0)
    {
        return 1.0;
    }

    const float2 shadowTextureCoordinate = lightNdcPosition.xy * 0.5 + 0.5;
    const float normalDotLight = saturate(dot(shadowNormal, lightDirection));
    ...
    return lerp(1.0, visibility / 9.0, saturate(lighting.directionalShadowParameters.z));
}

float3 EvaluatePBRLighting(float3 albedo, float metallic, float roughness, float ambientOcclusion, float emissiveStrength, float3 worldPosition, float3 normal, float3 shadowNormal)
{
    const float3 viewDirection = normalize(camera.position.xyz - worldPosition);
    ...
    const float directionalVisibility = CalculateDirectionalVisibility(worldPosition, shadowNormal, directionalLightDirection);
    ...
    const float3 pointOffset = lighting.pointPosition.xyz - worldPosition;
    ...
    const float3 emissiveLighting = albedo * max(emissiveStrength, 0.0);
    return ambientLighting + directionalLighting + pointLighting + emissiveLighting;
}
